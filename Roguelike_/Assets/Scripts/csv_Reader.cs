...
public class CSVReader<T> : IEnumerable<T>, IDisposable
    ...
    private void LoadType() {
        ...
        foreach (MemberInfo member in type.GetMembers(flag).Where((member) => memberTypes.Contains(member.MemberType))) {
            ...
            if (member.MemberType == MemberTypes.Field) {
                ...
                setters[columnIndex] = (target, value) => fieldInfo.SetValue(target, GetConvertedValue(fieldInfo, value, defaultValue));
            }
            else {
                // property
                PropertyInfo propertyInfo = type.GetProperty(member.Name, flag);

                setters[columnIndex] = (target, value) => propertyInfo.SetValue(target, GetConvertedValue(propertyInfo, value, defaultValue), null);
            }
        }
    }
    ...
    public CSVReader(string filePath) : this(filePath, true) {

    }

    public CSVReader(string filePath, bool skipFirstLine) : this(filePath, skipFirstLine, null) {
